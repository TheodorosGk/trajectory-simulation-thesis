...
# This script pretrains the road-level GAT-FC model.
...
temp_folder = os.path.join(save_root, "temp_gat")
...
# Porto grid size used during training
img_width = 28
img_height = 11
...
def collate_fn(indices):
    """
    Custom DataLoader collation function for road-level candidate prediction.
    """
    batch_des = []
    batch_candidate_set = []
    batch_candidate_dis = []
...
if train_model:
    ...
    for epoch in range(max_epoch):
        ...
        torch.save(gat.state_dict(), os.path.join(temp_folder, f"gat_{epoch}.pt"))
    ...
    load_temp_file = f"gat_{best_epoch}.pt"
